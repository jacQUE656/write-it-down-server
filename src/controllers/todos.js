...
async function quickAddTodo(req, res) {
  try {
...
async function listOpenTodos(req, res) {
  try {
...
async function getStalledTodos(req, res) {
  try {
...
async function setTodoDone(req, res) {
  try {
...
async function listTodosSharedWithMe(req, res) {
  try {
...
async function unshareTodo(req, res) {
  try {
    const { text, entryDate, email } = req.body;
    if (!text || !entryDate || !email) {
      return res.status(400).json({ error: 'text, entryDate, and email are required' });
